...
inline void cudaAssert(cudaError_t err, const char* __file, int __line) {
    if (cudaSuccess != err) {
        ...
        OUTPUTS("ERR: %s:%d  CUDA runtime error code=%d(%s) \"%s\" \n",__file,__line, err, cudaGetErrorName(err),cudaGetErrorString(err));
        exit(-1);
    }
}
...
inline void cuAssert(CUresult err, const char* __file, ssize_t __line) {
    if (CUDA_SUCCESS != err) {
        ...
        OUTPUTS("ERR: %s:%ld  CUDA driver error code=%d(%s) \"%s\" \n",__file, __line, err, errorName, errorDescription);
        exit(-1);
    }
}
...
//Kill program and print error message if a CUDA driver API fails.
...
/**
 * Before calling CUDA driver API, the initialization must be done here.
 * @return
 */
