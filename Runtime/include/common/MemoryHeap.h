...
namespace mlinsight {
    ...
    /**
     * A no-op memory heap that delegate all allocations and frees to system memory allocator.
    ...
    /**
     * A pre-allocated memory chunk that stores a group of slots. Each slot can hold one object of type T. Used in ObjectPoolHeap.
     */
    template<class T>
    class Chunk {
    protected:
    ...
    public:
        Chunk(ssize_t objNum) : nextChunk(nullptr), internalArrSize(objNum) {
            internalArray =(FreeListNode<T>*)(mmap(NULL, internalArrSize * sizeof(FreeListNode<T>), PROT_READ | PROT_WRITE,
                                                                                    MAP_PRIVATE | MAP_ANONYMOUS, -1, 0));
            if (internalArray == MAP_FAILED) {
                fatalErrorS("Failed to allocateArray memory for MemoryHeap::Chunk at %p because: %s", internalArray,
                            strerror(errno));
                exit(-1);
            }
        }
         ...
         */
        Chunk(const Chunk&& rho): internalArray(rho.internalArray), nextChunk(rho.nextChunk), internalArrSize(rho.internalArrSize),
        ...
        ~Chunk() {
            if(internalArray){
                if (munmap(internalArray, internalArrSize * sizeof(FreeListNode<T>)) == -1) {
                    fatalErrorS("Failed to deallocate memory for MemoryHeap::Chunk at %p because: %s", internalArray,
                                strerror(errno));
                    exit(-1);
                }
    ...
    public:
         ...
         * Since object pool contains all reserved memory, we do not need to copy actual memory from the original pool.
         * We only copy the size of rho and construct that many memory in one chunk.
         */
         ...
         * Copy and swap idiom: Move constructor.
         */
        ObjectPoolHeap(ObjectPoolHeap&& rho): largestChunkSize(rho.largestChunkSize), chunkHead(rho.chunkHead), freelistHead(rho.freelistHead),
                                              allocatedSize(rho.allocatedSize){
    ...
    protected:
        inline void expand(ssize_t targetSize) {
            ...
            if(!newChunk){
                fatalError("Cannot allocateArray memory in MemoryHeap.");
            }
